...
FORWARD _PROTOTYPE( int set_scheduler, (message *m_ptr) );
FORWARD _PROTOTYPE( int change_group, (message *m_ptr) );
FORWARD _PROTOTYPE( int get_group, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETSCHEDULER: r = set_scheduler(&m);	break;
	    case SYS_CHANGEGROUP: r = change_group(&m); break;
	    case SYS_GETGROUP:	r = get_group(&m);	break;
	    default:		r = E_BAD_FCN;
	}
...
PRIVATE int set_scheduler(m_ptr)
register message *m_ptr;
{
	if (bill_ptr->group == GROUP_B)
  {
    return -1;
  }
	else
...
PRIVATE int change_group(m_ptr)
register message *m_ptr;
{
	if (bill_ptr->group == GROUP_B)
	{
		bill_ptr->group = GROUP_A;
		return 0;
	}	
	else
	{
		bill_ptr->group = GROUP_B;
		return 0;
	}
...
PRIVATE int get_group(m_ptr)
register message *m_ptr;
{
  return bill_ptr->group;
}